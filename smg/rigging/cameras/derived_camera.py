...
class DerivedCamera(Camera):
    ...
    def __init__(self, base_camera: Camera, rot: np.ndarray, trans):
        ...
        """
        # : Camera
        self.__base_camera = base_camera
        # : np.ndarray
        self.__rot = rot
        # : np.ndarray
        self.__trans = np.array(trans, dtype=np.float64)
    ...
    def __make_world_space_rotation(self) -> np.ndarray:
        ...
        # : np.ndarray
        m = np.column_stack((self.__base_camera.u(), self.__base_camera.v(), self.__base_camera.n()))
